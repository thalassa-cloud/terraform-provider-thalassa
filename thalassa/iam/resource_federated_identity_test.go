...
func TestResourceFederatedIdentity(t *testing.T) {
	...
	t.Run("resource schema validation", func(t *testing.T) {
		...
		schema := resource.Schema
		assert.True(t, schema["name"].Required)
		assert.True(t, schema["service_account_identity"].Required)
		assert.True(t, schema["service_account_identity"].ForceNew)
		assert.True(t, schema["provider_identity"].Required)
		assert.True(t, schema["provider_identity"].ForceNew)
...
func TestDataSourceFederatedIdentity(t *testing.T) {
	...
	t.Run("data source schema validation", func(t *testing.T) {
		...
		assert.True(t, schema["id"].Computed)
		assert.True(t, schema["name"].Optional)
		assert.True(t, schema["service_account_identity"].Computed)
		assert.True(t, schema["provider_identity"].Computed)
