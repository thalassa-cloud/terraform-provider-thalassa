...
// GetKubernetesClusterKubeconfigWithParams is the same as GetKubernetesClusterKubeconfig but with optional parameters.
// This method returns a KubernetesClusterSessionToken containing all necessary information to connect to the cluster,
// including the API server URL, authentication token, CA certificate, and complete kubeconfig file content.
//
// Parameters:
...
//   - SessionLifetime: The lifetime of the session in seconds
//
// Returns:
//   - *KubernetesClusterSessionToken: Contains all cluster connection details
//   - error: Returns an error if the request fails, the cluster is not found, or the identity is invalid
...
//	defer cancel()
//
//	session, err := client.GetKubernetesClusterKubeconfigWithParams(ctx, "cluster-123", KubeconfigParams{SessionLifetime: 1 * time.Minute})
//	if err != nil {
//	    log.Fatalf("Failed to get kubeconfig: %v", err)
//	}
func (c *Client) GetKubernetesClusterKubeconfigWithParams(ctx context.Context, identity string, params KubeconfigParams) (*KubernetesClusterSessionToken, error) {
	// Validate input parameters
	if ctx == nil {
		return nil, fmt.Errorf("context cannot be nil")
	}
	...
	if params.SessionLifetime > 0 {
		queryParams.Add("sessionlifetime", params.SessionLifetime.String())
	}

	var clusterSession *KubernetesClusterSessionToken
	req := c.R().SetResult(&clusterSession).SetQueryParamsFromValues(queryParams)
	resp, err := c.Do(ctx, req, client.GET, fmt.Sprintf(KubernetesClusterKubeConfigEndpoint, identity))
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve kubeconfig for cluster %s: %w", identity, err)
	}
